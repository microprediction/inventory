...
w = 1/h ,
\]
which we call the \emph{market width} --- the reciprocal cover hazard estimated from the win curve, a one-sided competitive scale rather than the dealer's own quoted bid--offer width. Every won trade concedes a fixed amount $\epsilon$ to adverse selection: when we mark up an item by $m$ we assume we make only $m - \epsilon$. Holding inventory $x$ (a real number; negative values accommodate short positions) costs $c(x) \ge 0$ per unit time with $c(0) = 0$ --- the sign is an economic reading; the algebraic transformation below does not require it --- and the game is played forever: we study the ergodic, steady state policy.
...
\item \emph{Normalization.} Relative values are normalized by $\nu(0) = 0$. The theorem is a statement about solution sets and assumes no uniqueness; uniqueness of the balanced solution is invoked only where a corollary needs it (Corollaries~\ref{cor:zeroskew} and~\ref{cor:orders}), and the zero-carry equation's nonidentifiability is discussed in Remark~\ref{rem:zerocarry}.
...
\]
the balanced convexity's comparative static in the cost scale, for which Section~\ref{sec:closed} gives a closed small-skew approximation. The sign of the response follows the balanced convexity's cost-scale sensitivity: positive on the small-skew branch $hC_0 < 2$ with $C_0 > 0$, second order in general. This accords with the practitioner's instinct to skew into flow first and adjust width only when the market becomes seriously one-sided. The skew response involves no parameter beyond the observable width $w$; the width response involves in addition the balanced policy's cost-scale sensitivity.
\end{corollary}
