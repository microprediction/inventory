...
\label{eq:multiplier}
\end{equation}
under the correspondence: skew translated by $\delta$, and non-discretionary width $\Delta$ widened to $\Delta + \gamma$.
\end{theorem}
...
The theorem neither requires nor provides a closed form. In general $\nu$ must be computed numerically, and everything about its shape depends on $c$. What the symmetry provides is the correction: whatever the balanced solution is, the imbalanced one is that same object translated by $\delta$, widened by $\gamma$, and evaluated at carrying cost $M(q)\,c$. One balanced solve serves every imbalance.
\end{remark}
...
\]
A mild imbalance moves the midpoint linearly and the width only quadratically. This accords with the practitioner's instinct to skew into flow and widen only when the market becomes seriously one-sided. Both responses involve no parameter beyond the observable width $w$; the skew flips sign between net-selling and net-buying regimes while the widening, being even in $q - \tfrac12$, does not.
\end{corollary}
