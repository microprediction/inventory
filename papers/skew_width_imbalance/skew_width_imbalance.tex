...
Three standing assumptions scope what follows. The inventory state space is the lattice reached from the initial inventory, with boundary conditions (if any) compatible with the addition of a linear function of $x$; the carrying cost is coercive enough that the average-reward equation has a unique admissible relative-value solution $\nu$, up to an additive constant, with an ergodic inventory process (in particular $c \not\equiv 0$; the zero-carry edge is degenerate, per the remark below); and all optimal quotes on the states considered are interior, $m = w + K > 0$, in both the imbalanced problem and its balanced image --- a restriction that genuinely binds at extreme inventories, where the floor in \eqref{eq:foc} clips, and that the tilt makes bind earlier on one side. The equivalence is between relative values and policies, with the average reward scaling accordingly.
...
\label{eq:multiplier}
\end{equation}
Consequently $S_q(x) = \bar S(x) + \delta$ and $C_q(x) = \bar C(x)$, and on interior states the quotes map as
\[
m^{\downarrow}_q(x) = \bar m^{\downarrow}(x) + \delta, \qquad
m^{\uparrow}_q(x) = \bar m^{\uparrow}(x) - \delta :
\]
the physical width equals the balanced-at-$M(q)c$ width at every inventory, and the quote midpoint is translated by $-\delta$.
\end{theorem}
...
\]
which is the balanced consistency equation in the pair $(C, S_\delta)$, with the cost side inflated by $e^{h\gamma} = M(q)$. The ergodic constant, eliminated in \eqref{eq:bellman} by differencing against $x = 0$, scales by the same factor, giving the average-reward statement. The quote map follows from the interior first-order condition $m = w + K$ and $K = \epsilon + $ marginal $\nu$: the tilt adds $\delta$ to the buy-side marginal cost and subtracts it from the sell side, and the two shifts cancel in the width.
\end{proof}
...
\]
The skew's sign flip between net-selling and net-buying regimes needs no symmetry of the carrying cost.
\end{corollary}
...
\begin{remark}[The zero-carry edge is degenerate]\label{rem:zerocarry}
At $c \equiv 0$ the centered consistency equation is solved by \emph{every} linear tilt $\nu = bx$, so it selects no policy; the tilted image $\delta x$ of the balanced solution is moreover inadmissible as a liquidation cost on an unbounded lattice, being unbounded below. With admissibility restored the selected zero-carry policy does not skew: the per-enquiry problems decouple and each side quotes $w + \epsilon$. The flat-book skew $\delta$ therefore holds for every coercive $c$ but not in the $c \to 0$ limit, a cheap-control-type singularity of independent interest for goods with very low carrying costs.
\end{remark}
