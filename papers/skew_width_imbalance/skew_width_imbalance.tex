...
\begin{abstract}
A market maker fills sealed-bid enquiries that arrive with imbalanced intent: sellers more often than buyers, or the reverse. When the best competing quote is exponentially distributed around a commonly discerned fair price, the steady state problem facing imbalanced flow compresses exactly onto the balanced one: imbalance is absorbed by a translation of the skew together with a uniform slowing of the inventory clock --- equivalently, multiplication of the carrying cost by $1/(2\sqrt{q(1-q)})$. The correction involves no parameter beyond the observable market width, and the exponential assumption is needed only on the strikes the transformation traverses. Among the consequences: a dealer with zero inventory and symmetric carrying costs still skews; skew responds to imbalance at first order and width only at second; and constant-width linear-skew quoting reappears as the small-skew case of balanced flow with quadratic cost.
\end{abstract}
...
Trade opportunities arrive as a Poisson process with mean inter-arrival time $\tau$. Each arrival is a customer, met simultaneously by quotes from several market makers, and the trade is done at the winning quote. At each arrival there is a chance $q \in (0,1)$ that the customer is a seller (a \emph{buy opportunity} for our market maker) and chance $1-q$ that the customer is a buyer. All trades are of size $s$. There is a fair market price commonly discerned by all dealers; in the event of a buy opportunity our market maker submits a markdown $m^{\downarrow} > 0$ below it, and in the event of a sell opportunity a markup $m^{\uparrow} > 0$ above it. She wins if her response beats the best competing response, whose displacement beyond fair value has distribution function $F$, survival function $1 - F$, and hazard rate $h(z)$.

We take the hazard constant, so that the best competitor is exponentially distributed with mean
\[
w = 1/h ,
\]
which we call the \emph{market width}.\footnote{Remark~\ref{rem:local} weakens the constancy to the strike range the transformation traverses. Note that $w$ is the reciprocal cover hazard estimated from the win curve --- a one-sided competitive scale, not the dealer's own quoted bid--offer width.} Every won trade concedes a fixed amount $\epsilon$ to adverse selection: when we mark up an item by $m$ we assume we make only $m - \epsilon$.

Holding inventory $x$ costs $c(x) \ge 0$ per unit time with $c(0) = 0$.\footnote{Inventory is a real number, negative values accommodating short positions. The sign restriction on $c$ is an economic reading; the algebraic transformation below does not require it.} The game is played forever: we study the ergodic, steady state policy.
...
\begin{remark}[The fixed fair price is a frame, not a restriction]\label{rem:frame}
Suppose instead that the fair price $F_t$ is a square-integrable martingale with $d\langle F\rangle_t = \sigma^2\,dt$, independent of the enquiry and fill filtration, and quotes are made relative to it; inventory $x_{t-}$ is bounded and predictable. Cash profit then decomposes pathwise into the model's margin flow plus $M_T = \int_0^T x_{t-}\,dF_t$. The term $M_T$ has zero conditional expectation given any state and action, so it alters no Bellman comparison and selects the same policy; $\mathbb{E}[M_T^2] = \sigma^2\,\mathbb{E}\int_0^T x_t^2\,dt = O(T)$, so $M_T/T \to 0$ in $L^2$ and the long-run gain is unchanged. (Boundedness of $x$ alone would not suffice for an arbitrary martingale; the quadratic-variation rate is what controls $M_T$.)

A risk-averse dealer is not indifferent to the term's variance, which by the It\^o isometry is $\sigma^{2}\,\mathbb{E}[x^{2}]$ per unit time, with $\sigma^{2}$ the price-increment variance. Under a long-run mean--variance criterion (a variance-rate penalty with coefficient $\lambda$), the entire price risk of the moving frame is therefore the quadratic inventory charge $(\lambda\sigma^{2}/2)\,x^{2}$: a carrying cost $c$ of exactly the canonical form, arising as an identity rather than an assumption. The equivalence requires fair-price increments independent of fills; fill-correlated price moves and state-dependent adverse selection are outside the present model, and $\epsilon$ represents only the fixed reduced-form concession assumed here. The accompanying \texttt{verify\_moving\_price.py} checks the decomposition, the vanishing rate, the gain invariance for optimal and suboptimal policies, and the isometry.
\end{remark}
...
\]
for buying and selling respectively.\footnote{$K^{\uparrow}$ may well be negative for positive inventory: a loaded dealer will pay to get out.} The best response solves $\sup_m\, (m - K)\big(1-F(m)\big)$, and evidently the optimal choice of markup lies precisely where the increase in profit exactly offsets the possibility of losing the trade. For say we increased the markup by $\Delta m$; then
\[
...
\]
Every interior optimizer therefore satisfies $1 = (m - K)\,h(m)$, a necessary first-order condition independent of the distributional assumption on the inside market. When the objective is unimodal --- as it is under constant hazard --- this condition, together with comparison to the floor at $m = 0$, characterizes the best response:
\begin{equation}
...
\label{eq:quotes}
\end{equation}
Slope of the inventory cost is the skew; convexity of the inventory cost is the discretionary width. These identities are exact, and the second runs both ways: a dealer's quoting reveals the first two finite differences of her indifference value $\nu$, whether or not she has ever written it down --- and hence $\nu$ up to normalization, when enough inventory states are observed.\footnote{Recovering the primitive carrying cost $c$ from $\nu$ requires, in addition, the consistency equation and the model's arrival and win-curve parameters.}
...
\item \emph{Normalization.} Relative values are normalized by $\nu(0) = 0$. The theorem is a statement about solution sets and assumes no uniqueness.\footnote{Uniqueness of the balanced solution is invoked only where a corollary needs it (Corollaries~\ref{cor:zeroskew} and~\ref{cor:orders}); the zero-carry equation's nonidentifiability is Remark~\ref{rem:zerocarry}.}
...
\]
at every two-sided inventory state the physical width equals the balanced-at-$M(q)c$ width and the quote midpoint is translated by $-\delta$. The map $(\rho_q, \nu_q) \mapsto (M(q)\rho_q,\ \nu_q - \delta x)$ is thus a bijection between the normalized interior solution sets of the two problems; it assumes no uniqueness and remains valid at $c \equiv 0$, where $M(q)\,c = 0$.
\end{theorem}
...
At the certificate configuration itself the assumptions are not merely asserted: an interval-Newton (Krawczyk) certificate accompanying the paper proves existence, local uniqueness, and interiority of the solution and of its balanced image, with outward-rounded arithmetic --- pointwise, and by interval continuation for every $q \in [0.55, 0.605]$ at the certificate cost.
\end{remark}
...
\]
each balanced directional probability being $\tfrac12$. In generator form $\mathcal L_{q,c} = D(q)\,\mathcal L_{1/2,\,M(q)c}$ under the corresponding policies: the imbalanced inventory process has the same embedded jump chain, and (subject to positive recurrence) the same stationary inventory distribution, as the balanced-at-$M(q)c$ process, with its transition clock run at rate $D(q) \le 1$. Any positive calendar-time carrying cost is therefore amplified by $M(q) = 1/D(q)$ per effective inventory transition. Imbalance cannot create carrying cost when $c \equiv 0$: $M(q) \cdot 0 = 0$.\footnote{The formal multiplier diverges as $q \to 0$ or $1$, though the interior representation breaks down before the endpoints. The comparison is with the balanced dealer at cost $M(q)c$; relative to the balanced dealer at cost $c$ the policy itself also changes, so observed throughput is not simply rescaled by $D(q)$ against that benchmark.}
\end{corollary}
...
\]
Provided the balanced solution branch remains interior and locally unique for cost scales between $1$ and $M(q)$, and depends differentiably on the scale, a mild imbalance moves the midpoint linearly and the width only quadratically, and the width response flows entirely through the multiplied carry: at every two-sided state, exactly,
\[
W_q(x) - W_{1/2,c}(x) \;=\; 2\big[\,\bar C_{M(q)c}(x) - \bar C_{c}(x)\,\big],
\]
the balanced convexity's comparative static in the cost scale, for which Section~\ref{sec:closed} gives a closed small-skew approximation. This accords with the practitioner's instinct to skew into flow first and adjust width only when the market becomes seriously one-sided.\footnote{The sign of the response follows the balanced convexity's cost-scale sensitivity: positive on the small-skew branch $hC_0 < 2$ with $C_0 > 0$, second order in general. The skew response involves no parameter beyond the observable width $w$; the width response involves in addition this cost-scale sensitivity.}
\end{corollary}

\begin{remark}[Zero carry: the interior equations alone are nonidentifying]\label{rem:zerocarry}
At $c \equiv 0$ the centered two-sided interior equations alone are nonidentifying: every linear tilt $\nu = bx$ whose quotes remain interior solves them. This does not invalidate Theorem~\ref{thm:main}, which concerns the full finite-lattice average-reward equation and maps zero-carry solutions to zero-carry solutions --- $M(q) \cdot 0 = 0$, so imbalance does not manufacture carrying cost from nothing. On the finite lattice the one-sided boundary rows select a branch; in the configurations studied numerically they select the optimal tilted branch, with $S_q(0) = \delta$. On an unbounded domain an additional recurrence or transversality condition would be required instead.
\end{remark}
...
\]
Exactness requires $\log G$ to be affine over the translated strike ranges used by the visited inventory states; constant optimizer hazard throughout those ranges is sufficient, and observing the hazard at finitely many quotes is not. If the hazard varies slowly there and the quotes remain interior, $\log G$ is approximately affine, suggesting an $O(\eta)$ perturbation of the exponential symmetry with $\eta$ the hazard's relative variation; a stability estimate for the consistency equation, which we do not provide here, would be required to make this exact. We checked the scaling numerically in \texttt{verify\_local\_exponentiality.py}, which accompanies the paper: the error in the zero-inventory skew is roughly a quarter to a third of the hazard's relative variation across the visited strikes, and halving the perturbation multiplies the error by $1.6$--$1.9$, consistent with first-order scaling.
\end{remark}
...
\emph{Fill ratios.} At the optimum, profitability factors cleanly. By \eqref{eq:foc} the optimal response sits the distance $w$ beyond its strike, so the \emph{continuation-adjusted surplus} on a won trade --- the one-step Bellman value, not the cash margin $s(m - \epsilon)$, which varies with inventory through the continuation term --- is $s\,w$ at every level of inventory, and all variation in the one-step Bellman value per enquiry is variation in the fill ratio. Under the constant hazard the fill ratio on a quote at markup $m$ is $e^{-m/w}$, log-linear in the markup with slope $-1/w$, which is how $w$ is estimated from a quote archive; at the optimal response to strike $K$ it equals $e^{-1-K/w}$. In particular a dealer facing zero adverse selection and zero marginal inventory cost should, if she quotes optimally, win $e^{-1} \approx 37\%$ of her enquiries, and persistent departures from that ratio measure her effective strike.
