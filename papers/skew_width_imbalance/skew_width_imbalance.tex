...
Three standing assumptions scope what follows. The inventory state space is the finite lattice $\{-Ns, \dots, Ns\}$ reached from the initial inventory, with no inventory-increasing trade offered at the corresponding boundary, so the average-reward problem is a finite-state MDP; the carrying cost is not identically zero (the zero-carry equation is degenerate, per the remark below) and the average-reward equation has a unique relative-value solution $\nu$ up to an additive constant, normalized by $\nu(0) = 0$ --- here $-\nu$ is a relative value that may take either sign, the liquidation-cost reading of Section~\ref{sec:nu} being the motivating interpretation; and every \emph{available} quote is interior, $m = w + K > 0$, in both the imbalanced problem and its balanced image, a restriction that genuinely binds at extreme inventories, where the floor in \eqref{eq:foc} clips, and which the tilt makes bind earlier on one side. The equivalence is between relative values and policies, with the average reward scaling accordingly.
