...
The market maker can remain indifferent in two ways: liquidate now and re-enter, or carry to the next opportunity. Comparing the two yields a functional equation for $\nu$. With exponential markups, $\sup_m (m-K)e^{-hm} = \tfrac{1}{h}e^{-1-hK}$, In average-reward form, with $-\nu$ the relative value normalized by $\nu(0) = 0$ and $\rho_q$ the gain, the equation is $\rho_q = -c(x) + \tfrac{s}{\tau}\big[q\,G(K^{\downarrow}(x)) + (1-q)\,G(K^{\uparrow}(x))\big]$; subtracting its value at $x = 0$ gives the centered form used below. The enquiry value is $we^{-1-hK}$ on the interior branch $K \ge -w$ only (below it the floor gives $G(K) = -K$), so the steady state condition takes the form: for every interior state under consideration,
\begin{equation}
\frac{\tau\, c(x)}{s}
...
Three standing assumptions scope what follows. The inventory state space is the finite lattice $\{-Ns, \dots, Ns\}$ reached from the initial inventory, with no inventory-increasing trade offered at the corresponding boundary, so the average-reward problem is a finite-state MDP; the carrying cost is not identically zero (the zero-carry equation is degenerate, per the remark below) and the average-reward equation has a unique relative-value solution $\nu$ up to an additive constant, normalized by $\nu(0) = 0$ --- here $-\nu$ is a relative value that may take either sign, the liquidation-cost reading of Section~\ref{sec:nu} being the motivating interpretation; and every \emph{available} quote is interior, $m = w + K > 0$, in both the imbalanced problem and its balanced image, a restriction that genuinely binds at extreme inventories, where the floor in \eqref{eq:foc} clips, and which the tilt makes bind earlier on one side. It binds first at the boundary state itself: only one side trades there, so the relative value of escaping it can exceed $w + \epsilon - \delta$ once the imbalance is strong, clipping the optimal boundary sell quote to zero at \emph{every} cost level, including $c \to 0$. The admissible region of $(q, N, \epsilon, c)$ is accordingly small, and the accompanying certificates assert membership rather than assume it. The equivalence is between relative values and policies, with the average reward scaling accordingly.
