...
Suppose instead that the fair price $F_t$ is a martingale independent of the enquiry process, and quotes are made relative to it. Cash profit then decomposes pathwise into the model's margin flow plus the inventory-weighted price increment $\int x\,dF$. That term has zero conditional expectation given any state and action, so it alters no Bellman comparison and selects the same policy; on the bounded lattice its time average vanishes, and the gain is unchanged.

A risk-averse dealer is not indifferent to the term's variance, which by the It\^o isometry is $\sigma^{2}\,\mathbb{E}[x^{2}]$ per unit time, with $\sigma^{2}$ the price-increment variance. For a mean--variance dealer with risk aversion $\lambda$, the entire price risk of the moving frame is therefore the quadratic inventory charge $(\lambda\sigma^{2}/2)\,x^{2}$: a carrying cost $c$ of exactly the canonical form, arising as an identity rather than an assumption. The reduction requires price increments independent of fills; flow that moves the price is adverse selection, entering here only through the constant $\epsilon$. The accompanying \texttt{verify\_moving\_price.py} checks the decomposition, the vanishing rate, the gain invariance for optimal and suboptimal policies, and the isometry.
\end{remark}
...
\label{eq:quotes}
\end{equation}
Slope of the inventory cost is the skew; convexity of the inventory cost is the discretionary width. These identities are exact, and the second runs both ways: a dealer's quoting reveals the first two derivatives of her indifference value $\nu$, whether or not she has ever written it down. Recovering the primitive carrying cost $c$ from them requires, in addition, the consistency equation and the model's arrival and win-curve assumptions.
